...
class MSELossNode(MetaNode):
    """A mean squared error loss node for one prediction: J = 1/2 * (y_hat - y)^2."""
    ...
    def __init__(self, y_hat: ValueNode, y: ValueNode, out: ValueNode):
        ...
        Args:
            ...
            y: True target value node.
            out: Output node receiving the loss value.
    ...
    def forward(self):
        """Compute MSE loss and push it to children."""
        if self.input_ready:
    ...
    def backward(self, grad_z):
        """Propagate gradients of J = 1/2 * (y_hat - y)^2."""
        y_hat_val, y_val = self.get_parent_values()
        error = y_hat_val - y_val

        self.parents[0].backward(grad_z * error)
        self.parents[1].backward(grad_z * -error)
